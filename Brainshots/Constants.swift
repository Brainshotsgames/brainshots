...
let IsHost = UserDefaults.standard.bool(forKey: "isHost")
let kBaseRed = UIColor.init(colorLiteralRed: 176.0/255.0, green: 26.0/255.0, blue: 21.0/255.0, alpha: 1)
let email = "info@brainshotsgames.com"
let link = "http://brainshotsgames.com"
...
//typealias commonMethod = CommonFunctions.sharedInstance
...
func setPopupAnimation2(target:UIView){
...
let Individual_Play_Text = "After all players login and “shots” are chosen, you are ready to play.  The game chooses the first player.  This player must sing a song using the random word on the screen in one or two lines.  If a player successfully sings a song with that word before the 10 seconds are up, that player swipes right for a point.  Play continues with the next player who uses the same word in a new song.  If the time runs out before a song is sung, the player is out of the round and must take their shot.The round is over when everyone has run out of new songs to sing."

let Individual_Play_Continued = "The last player to successfully sing a song is the winner of the round, gets an additional point and does not have to take a shot.The new round begins with player 2 and everyone is back in for the next \"random word.\"  No song may be repeated in the same round.  You can use a variation of the word (eg. love, lover, loving). You must 'sing' the song, no talking!  Once per game, each player is randomly selected for a lightning round to gain extra points (as many songs as you can sing in a category in 30 seconds)."
...
let wordPackProducts = ["com.paul.brainshots.love","com.paul.brainshots.rockandroll","com.paul.brainshots.earthandspace","com.paul.brainshots.hiphop","com.paul.brainshots.countrywestern","com.paul.brainshots.wordstew"]
...
extension UIView {
    ...
    func morph() {
        ...
        UIView.animateKeyframes(withDuration: 0.1, delay: 0, options: UIViewKeyframeAnimationOptions(rawValue: 0), animations: {
            ...
            UIView.animateKeyframes(withDuration: 0.1, delay: 0, options: UIViewKeyframeAnimationOptions(rawValue: 0), animations: {
                ...
                self.transform = CGAffineTransform.init(scaleX: 1.1, y: 0.5)
            }) {
                (f) in
