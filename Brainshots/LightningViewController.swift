...
class LightningViewController: UIViewController,UIGestureRecognizerDelegate,IPSessionDelegate {
    ...
    var repeatLightningRound =  Bool()
   // var checkMarkTimer = Timer()
    var showMark = Bool()
...
//        
//        self.imgCheckMark.alpha = 0
//        perform(#selector(startRound), with: nil, afterDelay: 2)
        
    }
    
    //MArk:- Show Check mark
    func showCheckMark() {
    ...
    func selectRandomPlayer() {
        ...
        if(array.count == 1) {
            ...
            CommonFunctions.sharedInstance.fillLightningRoundCount()
            
            let gamePoints : NSInteger = CommonFunctions.sharedInstance.getGamePoints()
            
            print("Lightning Round Count ---> \(CommonFunctions.sharedInstance.lightningCount)")
            print("Game Points ---> \(gamePoints)")

            if(gamePoints == 50) {

                if(CommonFunctions.sharedInstance.lightningCount == 2){
        ...
        //Check for Lightning Round repetition
        if(repeatLightningRound){
            fillRandomPlayer()
        }
        else{
...
//                }
    ...
    func swipeAction() {
        ...
        let jsonObject  = ["message":"swipe"] as [String : String]
...
//        self.playWord = "Love"
    ...
    func popResult() {
        ...
        /*Results to show*/
        let str : String = messageToDisplay()
        lblResult.text = str
    ...
    func goodGuessSound() {
        ...
        do {
            ...
            goodGuessBell.setVolume(1, fadeDuration: 0)
